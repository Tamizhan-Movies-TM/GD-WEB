...
function pathToId(path) {
    if (!pathMap.has(path)) {
        const id = (++pathCounter).toString(36) + Date.now().toString(36).slice(-6);
        pathMap.set(path, id);
    }
...
function idToPath(id) {
    for (const [path, pathId] of pathMap) {
        if (pathId === id) return path;
    }
    return null;
}

function updateUrlForItem(itemPath, isFile = false) {
    const id = pathToId(itemPath);
    const newPath = `/${window.current_drive_order}:/view?=${isFile ? 'file' : 'folder'}/${id}`;
    history.pushState(null, null, newPath);
}
// ========================================================================

const folder_icon = `<svg xmlns="http://www.w3.org/2000/svg" width="1.5em" height="1.5em" viewBox="0 0 48 48" preserveAspectRatio="xMidYMid meet"><g clip-path="url(#__lottie_element_11)"><g transform="matrix(1,0,0,1,0,0)" opacity="1" style="display: block;"><g opacity="1" transform="matrix(1,0,0,1,24,24)"><path fill="rgb(255,159,0)" fill-opacity="1" d=" M16,-12 C16,-12 -2,-12 -2,-12 C-2,-12 -6,-16 -6,-16 C-6,-16 -16,-16 -16,-16 C-18.200000762939453,-16 -20,-14.199999809265137 -20,-12 C-20,-12 -20,12 -20,12 C-20,14.208999633789062 -18.208999633789062,16 -16,16 C-16,16 13.682000160217285,16 13.682000160217285,16 C13.682000160217285,16 20,5 20,5 C20,5 20,-8 20,-8 C20,-10.199999809265137 18.200000762939453,-12 16,-12z"></path></g></g><g transform="matrix(1,0,0,1,24,26)"><path fill="rgb(255,201,40)" fill-opacity="1" d=" M16,-14 C16,-14 -16,-14 -16,-14 C-18.200000762939453,-14 -20,-12.199999809265137 -20,-10 C-20,-10 -20,10 -20,10 C-20,12.199999809265137 -18.200000762939453,14 -16,14 C-16,14 16,14 16,14 C18.200000762939453,14 20,12.199999809265137 20,10 C20,10 20,-10 20,-10 C20,-12.199999809265137 18.200000762939453,-14 16,-14z"></path></g></g></svg>`
...
function render(path) {
    ...
    const fileMatch = path.match(/\/view\?=file\/([^\/]+)/);
    if (fileMatch) {
        const fileId = fileMatch[1];
        const realPath = idToPath(fileId);
        return realPath ? file(realPath) : fallback(fileId, true);
    }

    const folderMatch = path.match(/\/view\?=folder\/([^\/]+)/);
    if (folderMatch) {
        const folderId = folderMatch[1];
        const realPath = idToPath(folderId);
        return realPath ? list(realPath) : list(null, folderId, true);
    }
    ...
    try {
        ...
        for (i in files) {
            ...
            if (item['mimeType'] == 'application/vnd.google-apps.folder') {
                html += `<a href="/${window.current_drive_order}:/view?=folder/${pathToId(p)}" style="color: ${UI.folder_text_color};" class="countitems list-group-item list-group-item-action"> ${folder_icon} ${item.name} ${UI.display_time ? `<span class="badge bg-info float-end"> ` + item['modifiedTime'] + ` </span>` : ``}</a>`;
            } else {
...
function append_files_to_list(path, files) {
    ...
    for (i in files) {
        ...
        if (item['mimeType'] == 'application/vnd.google-apps.folder') {
            ...
            html += ` <a class="countitems size_items list-group-item-action" style="text-decoration: none; color: ${UI.css_a_tag_color};" href="/${window.current_drive_order}:/view?=file/${pathToId(pn)}">${item.name}</a>${UI.display_download ? `<a href="${link}"><svg class="float-end"width="25px" style="margin-left: 8px;" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"> <path d="M.5 9.9a.5.5 0 0 1 .5.5v2.5a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-2.5a.5.5 0 0 1 1 0v2.5a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2v-2.5a.5.5 0 0 1 .5-.5z"></path> <path d="M7.646 11.854a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 10.293V1.5a.5.5 0 0 0-1 0v8.793L5.354 8.146a.5.5 0 1 0-.708.708l3 3z"></path> </svg></a>` : ``}${UI.display_size ? `<span class="badge bg-primary float-end"> ` + item['size'] + ` </span>` : ``}${UI.display_time ? ` <span class="badge bg-info float-end"> ` + item['modifiedTime'] + ` </span>` : ``}</div>`;
        }
    }
    if (is_file && UI.allow_selecting_files) {
        document.getElementById('select_items').style.display = 'block';
