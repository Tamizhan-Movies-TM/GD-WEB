    ...
    <div class="card">
      ...
      <div class="col-md-12">
        <div class="text-center">
          <p class="mb-2">🚀&nbsp;𝔽𝕒𝕤𝕥&nbsp;&nbsp;𝔻𝕠𝕨𝕟𝕝𝕠𝕒𝕕&nbsp;&nbsp;𝔾𝕂𝕐&nbsp;&nbsp;𝕃𝕚𝕟𝕜&nbsp;&nbsp;<i class="fa-solid fa-cloud-arrow-down"></i></p>
          <div class="btn-group text-center">
            ${UI.display_drive_link ? `
           <button class="btn btn-secondary d-flex align-items-center gap-2 gdflix-btn"
          data-file-id="${file_id}" type="button">${gdrive_icon}𝗚𝗞𝗬 𝗟𝗶𝗻𝗸</button>` : ``}
...
// GDFlix button now uses GKYFileHost as backend (GDFlix is down)
function generateGDFlixLink(fileId) {
    return new Promise((resolve, reject) => {
        log('GKYFileHost (via GDFlix btn) - Received fileId:', fileId);
        ...
        // Safari fix: open blank tab synchronously inside the user-gesture stack
        const _isSafari = /^((?!chrome|android).)*safari/i.test(navigator.userAgent);
        var newTab = _isSafari ? window.open('', '_blank') : null;

        // Reuse the /gkyfilehost worker endpoint
        fetch('/gkyfilehost', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        .then(data => {
            log('GKYFileHost - Worker response:', data);

            const link = data.link || data.gkyfilehost_link;
            if (data.success && link) {
                log('GKYFileHost - Generated link:', link);
                if (_isSafari) {
