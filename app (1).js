// ... [redesigned by telegram.dog/TheFirstSpeedster ...] ...
// v2.3.8
// Initialize the page
function init() {
...
const image_icon = `<svg xmlns="http://www.w3.org/2000/svg" width="1.5em" height="1.5em" viewBox="0 0 48 48" preserveAspectRatio="xMidYMid meet"><g clip-path="url(#__lottie_element_19)"><g transform="matrix(1,0,0,1,24,24)" opacity="1" style="display: block;"><g opacity="1" transform="matrix(1,0,0,1,0,0)"><path fill="rgb(245,124,0)" fill-opacity="1" d=" M16,17 C16,17 -16,17 -16,17 C-16,17 -16,-13 -16,-13 C-16,-15.199999809265137 -14.199999809265137,-17 -12,-17 C-12,-17 12,-17 12,-17 C14.199999809265137,-17 16,-15.199999809265137 16,-13 C16,-13 16,13 16,13 C16,15.199999809265137 14.199999809265137,17 12,17z"></path></g></g><g transform="matrix(1,0,0,1,35,16)" opacity="1" style="display: block;"><g opacity="1" transform="matrix(1,0,0,1,0,0)"><path fill="rgb(255,249,196)" fill-opacity="1" d=" M0,-3 C-1.656999945640564,-3 -3,-1.656999945640564 -3,0 C-3,1.656999945640564 -1.656999945640564,3 0,3 C1.656999945640564,3 3,1.656999945640564 3,0 C3,-1.656999945640564 1.656999945640564,-3 0,-3z"></path></g></g><g transform="matrix(1,0,0,1,0,0)" opacity="1" style="display: block;"><g opacity="1" transform="matrix(1,0,0,1,20,24)"><path fill="rgb(148,42,9)" fill-opacity="1" d=" M0,-8 C0,-8 -11,8 -11,8 C-11,8 11,8 11,8 C11,8 0,-8 0,-8z"></path></g><g opacity="1" transform="matrix(1,0,0,1,31,27)"><path fill="rgb(191,54,12)" fill-opacity="1" d=" M0,-5 C0,-5 -8,5 -8,5 C-8,5 8,5 8,5 C8,5 0,-5 0,-5z"></path></g></g></g></svg>`
...
// ====== START OF URL MASKING IMPLEMENTATION ====== //
function render(path) {
    if (path.indexOf("?") > 0) {
        path = path.substr(0, path.indexOf("?"));
    }
    
    // Handle masked URL patterns
    if (path.startsWith('/view?=folder/')) {
        const folderId = path.split('/').pop();
        return fallback(folderId, false);
    }
    else if (path.startsWith('/view?=file/')) {
        const fileId = path.split('/').pop();
        return fallback(fileId, true);
    }
...
// ====== END OF URL MASKING IMPLEMENTATION ====== //
...
// ... [sleep function remains the same] ...
...
// ... [other functions remain the same until list function] ...
...
// ... [askPassword function remains the same] ...
...
function append_files_to_fallback_list(path, files) {
    try {
        ...
        for (i in files) {
            var item = files[i];
            // ====== URL MASKING FOR FILES AND FOLDERS ====== //
            if (item['mimeType'] == 'application/vnd.google-apps.folder') {
                html += `<a href="/view?=folder/${item.id}" style="color: ${UI.folder_text_color};" class="countitems list-group-item list-group-item-action"> ${folder_icon} ${item.name} ${UI.display_time ? `<span class="badge bg-info float-end"> ` + item['modifiedTime'] + ` </span>` : ``}</a>`;
            } else {
...
// ... [render_search_result_list function remains the same] ...
...
function append_search_result_to_list(files) {
    try {
        ...
        for (i in files) {
            ...
            // ====== URL MASKING FOR SEARCH RESULTS ====== //
            if (item['mimeType'] == 'application/vnd.google-apps.folder') {
                ...
                // ====== USE MASKED URL FOR SEARCH RESULT FILES ====== //
...
function onSearchResultItemClick(file_id, can_preview) {
    ...
    fetch(`/${cur}:id2path`, {
        ...
        .then(function(obj) {
            title = `Result`;
            $('#SearchModelLabel').html(title);
            // ====== USE MASKED URL IN SEARCH RESULTS ====== //
            if (can_preview) {
                content = `<a class="btn btn-info" href="/view?=file/${file_id}">Open</a> <a class="btn btn-secondary" href="/view?=file/${file_id}" target="_blank">Open in New Tab</a>`;
            } else {
                content = `<a class="btn btn-info" href="/view?=folder/${file_id}">Open</a> <a class="btn btn-secondary" href="/view?=folder/${file_id}" target="_blank">Open in New Tab</a>`;
            }
            $('#modal-body-space').html(content);
        })
        .catch(function(error) {
