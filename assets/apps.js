...
// Check if user is logged in by verifying session cookie structure.
//
// WHY THIS EXISTS — and what it does / does not do:
//   The server performs full cryptographic session validation (AES-CBC decrypt +
//   HMAC integrity check + expiry check + password-hash match) on every request.
//   This client-side check is purely a UI gate — it controls which buttons are
//   rendered (direct download vs GKYFILEHOST, Chrome link vs ad shortlinks).
//
// WHAT THE OLD CODE DID WRONG:
//   Old: checked only that a cookie named "session" exists and isn't empty.
//   Anyone could run:  document.cookie = "session=x"  in DevTools and instantly
//   appear "logged in" to the frontend — skipping monetisation shortlinks and
//   getting direct download buttons (even though the server would still reject
//   any actual API call made with a fake cookie).
//
// WHAT THIS FIX DOES:
//   The real session cookie is built by the worker as four AES-encrypted segments
//   joined by pipes:  enc(username)|enc(password)|enc(expiry)|enc(passwordHash)
//   Each encrypted segment is a hex string of at least 32 characters.
//   A fake cookie like "session=x" or "session=anything" has no pipes and is far
//   too short, so it fails the structural check below.
//
//   This does NOT replace server-side auth — it only raises the bar for the
//   trivial one-line DevTools bypass that bypasses UI-level routing.
//
function isUserLoggedIn() {
    try {
        const cookies = document.cookie.split(';');
        for (let cookie of cookies) {
            // Split only on the FIRST '=' so base64/hex values with '=' are safe
            ...
            // Reject obviously fake / empty values
            if (!value || value === 'null' || value === 'undefined' || value === '') continue;

            // ── Structural validation ────────────────────────────────────────
            // Real session format:  seg0|seg1|seg2|seg3
            // Each seg is an AES-CBC hex output — minimum ~32 hex chars each.
            // Total minimum length: 4 segs × 32 chars + 3 pipes = 131 chars.
            // A manually typed fake value will almost never pass this.
            const parts = value.split('|');
            if (parts.length !== 4) {
                log('Session cookie has wrong segment count:', parts.length, '(expected 4)');
                continue;
            }

            const MIN_SEG_LEN = 32; // AES-CBC output is always >= 32 hex chars
            const allSegsValid = parts.every(seg =>
                seg.length >= MIN_SEG_LEN && /^[0-9a-fA-F]+$/.test(seg)
            );
            if (!allSegsValid) {
                log('Session cookie segments failed hex/length check');
                continue;
            }

            // Passed structural check — treat as logged in for UI purposes.
            // The server will still cryptographically verify this on every API call.
            log('Session cookie passed structural check — rendering logged-in UI');
            return true;
        }
    } catch (err) {
        log('isUserLoggedIn error:', err);
    }
    log('No valid session cookie found — rendering logged-out UI');
    return false;
}
