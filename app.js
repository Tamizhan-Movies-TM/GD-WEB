    ...
    try {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            if (item['mimeType'] == 'application/vnd.google-apps.folder') {
                html += `<div class="list-group-item list-group-item-action d-flex align-items-center flex-md-nowrap flex-wrap justify-sm-content-between column-gap-2"><a href="${p}" style="color: ${UI.folder_text_color};" class="countitems w-100 d-flex align-items-start align-items-xl-center gap-2"><span>${folder_icon}</span>${escapeHtml(item.name)}</a>${UI.display_time ? `<span class="badge bg-info" style="margin-left: 2rem;">` + item['createdTime'] + `</span>` : ``}${UI.display_size ? `<span class="badge my-1 text-center" style="min-width: fit-content; padding: 4px 12px; background: transparent !important; border: 2.5px solid #00e676; color: #ffffff;">—</span>` : ``}<span class="d-flex gap-2">
                ...
                html += `</span>${escapeHtml(item.name)}</a>${UI.display_time ? `<span class="badge bg-info" style="margin-left: 2rem;">` + item['createdTime'] + `</span>` : ``}${UI.display_size ? `<span class="badge my-1 text-center" style="min-width: fit-content; padding: 4px 12px; background: transparent !important; border: 2.5px solid #00e676; color: #ffffff;">` + item['size'] + `</span>` : ``}<span class="d-flex gap-2">
