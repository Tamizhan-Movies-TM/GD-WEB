    ...
    try {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            if (item['mimeType'] == 'application/vnd.google-apps.folder') {
                html += `<div class="list-group-item list-group-item-action d-flex align-items-center flex-md-nowrap flex-wrap justify-sm-content-between column-gap-2"><a href="${p}" style="color: ${UI.folder_text_color};" class="countitems w-100 d-flex align-items-start align-items-xl-center gap-2" style="min-width:0;"><span class="flex-shrink-0">${folder_icon}</span><span class="text-truncate">${escapeHtml(item.name)}</span></a>${UI.display_time ? `<span class="badge bg-info" style="margin-left: 2rem;">` + item['createdTime'] + `</span>` : ``}${UI.display_size ? `<span class="badge my-1 text-center" style="min-width: fit-content; padding: 4px 12px; background: transparent !important; border: 2.5px solid #00e676; color: #ffffff;">—</span>` : ``}<span class="d-flex gap-2">
                ...
                c += " view";
                //}
                html += `<div class="list-group-item list-group-item-action d-flex align-items-center flex-md-nowrap flex-wrap justify-sm-content-between column-gap-2">${UI.allow_selecting_files ? '<input class="form-check-input" style="margin-top: 0.3em;margin-right: 0.5em;" type="checkbox" value="'+link+'" id="flexCheckDefault">' : ''}<a class="countitems size_items w-100 d-flex align-items-start align-items-xl-center gap-2" style="text-decoration: none; color: ${UI.css_a_tag_color}; min-width:0;" href="${p}&a=view"><span class="flex-shrink-0">`

                html += _getIcon(ext, item.mimeType, item.iconLink);

                html += `</span><span class="text-truncate">${escapeHtml(item.name)}</span></a>${UI.display_time ? `<span class="badge bg-info" style="margin-left: 2rem;">` + item['createdTime'] + `</span>` : ``}${UI.display_size ? `<span class="badge my-1 text-center" style="min-width: fit-content; padding: 4px 12px; background: transparent !important; border: 2.5px solid #00e676; color: #ffffff;">` + item['size'] + `</span>` : ``}<span class="d-flex gap-2">
...
function append_files_to_list(path, files) {
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        // replace / with %2F
        if (item['mimeType'] == 'application/vnd.google-apps.folder') {
            html += `<div class="list-group-item list-group-item-action d-flex align-items-center flex-md-nowrap flex-wrap justify-sm-content-between column-gap-2"><a href="${p}" style="color: ${UI.folder_text_color};" class="countitems w-100 d-flex align-items-start align-items-xl-center gap-2" style="min-width:0;"><span class="flex-shrink-0">${folder_icon}</span><span class="text-truncate">${escapeHtml(item.name)}</span></a>${UI.display_time ? `<span class="badge bg-info" style="margin-left: 2rem;">` + item['createdTime'] + `</span>` : ``}<span class="d-flex gap-2">
            ${UI.display_download ? `<a class="d-flex align-items-center" href="${p}" title="via Index"><i class="far fa-folder-open fa-lg"></i></a>` : ``}</span></div>`;
        } else {
            totalsize = totalsize + Number(item.size || 0);
            item['size'] = formatFileSize(item['size']) || '—';
            ...
            c += " view";
            //}
            html += `<div class="list-group-item list-group-item-action d-flex align-items-center flex-md-nowrap flex-wrap justify-sm-content-between column-gap-2">${UI.allow_selecting_files ? '<input class="form-check-input" style="margin-top: 0.3em;margin-right: 0.5em;" type="checkbox" value="'+link+'" id="flexCheckDefault">' : ''}<a class="countitems size_items w-100 d-flex align-items-start align-items-xl-center gap-2" style="text-decoration: none; color: ${UI.css_a_tag_color}; min-width:0;" href="${pn}"><span class="flex-shrink-0">`
