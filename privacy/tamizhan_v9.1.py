...
# ── Language filters ──────────────────────────────────────────────────
# Audio tracks: keep only these languages (others are dropped)
KEEP_AUDIO_LANGS = {"tam", "tel", "mal", "kan", "hin", "eng", "kor"}
# Subtitle tracks: keep only these languages (others are dropped)
KEEP_SUB_LANGS   = {"tam", "tel", "mal", "kan", "hin", "eng", "kor"}
...
def _tracks_need_fix(fp, sorted_audio, eng_subs):
    ...
    # ── 1. Audio count must match (only kept languages) ──────────────
    kept_a_on_disk = [t for t in a_tracks
                      if _norm_lang(t["language"]) in KEEP_AUDIO_LANGS]
    if len(kept_a_on_disk) != len(sorted_audio):
        return True, (f"audio count mismatch "
                      f"({len(kept_a_on_disk)} kept on disk vs {len(sorted_audio)} expected)")

    # ── 2. Language order + default flag ─────────────────────────────
    kept_a_sorted = sorted(kept_a_on_disk, key=lambda t: _lang_pri(t["language"]))
    for i, (on_disk, expected) in enumerate(zip(kept_a_sorted, sorted_audio)):
    ...
    # ── 3. Subtitle count must match (only kept languages) ───────────
    kept_s_on_disk = [s for s in s_tracks
                      if _norm_lang(s.get("language", "und")) in KEEP_SUB_LANGS]
    if len(kept_s_on_disk) != len(eng_subs):
        return True, (f"sub count mismatch "
                      f"({len(kept_s_on_disk)} kept on disk vs {len(eng_subs)} expected)")

    # ── 4. Subtitle language tags ─────────────────────────────────────
    kept_s_sorted = sorted(kept_s_on_disk, key=lambda s: _lang_pri(s.get("language", "und")))
    for i, (on_disk, expected) in enumerate(zip(kept_s_sorted, eng_subs)):
...
def _process_video_files_from_dir(extract_dir, folder_name, new_folder_id, tok,
    ...
    for i, vf in enumerate(video_files):
        ...
        filtered_a    = [t for t in aud
                         if _norm_lang(t["language"]) in KEEP_AUDIO_LANGS]
        sorted_a      = sorted(filtered_a, key=lambda t: _lang_pri(t["language"]))
        eng_s         = sorted(
                            [s for s in subs
                             if _norm_lang(s.get("language", "und")) in KEEP_SUB_LANGS],
                            key=lambda s: _lang_pri(s.get("language", "und"))
                        )
    ...
    for vi, vf in enumerate(video_files):
        ...
        if not sorted_a:
            print(f"  {R}⚠ No matching audio tracks after language filter — skipped.{X}")
            fail += 1
            vf.unlink(missing_ok=True)
            continue
        ...
        kept_langs = " + ".join(t["language"] for t in sorted_a)
        print(f"  {C}⚙ Remux [{engine}]{X}  Audio: {len(sorted_a)} tracks [{kept_langs}]  "
              f"Subs: {len(eng_s)}")
...
def auto_fix_v5(fp, known_fid=None, drive_dest=None):
    ...
    filtered_audio = [t for t in before_audio
                      if _norm_lang(t["language"]) in KEEP_AUDIO_LANGS]
    sorted_audio   = sorted(filtered_audio, key=lambda t: _lang_pri(t["language"]))
    eng_subs       = sorted(
                        [s for s in before_subs
                         if _norm_lang(s.get("language", "und")) in KEEP_SUB_LANGS],
                        key=lambda s: _lang_pri(s.get("language", "und"))
                     )
    ...
    dropped_audio  = [t for t in before_audio
                      if _norm_lang(t["language"]) not in KEEP_AUDIO_LANGS]
    dropped_subs   = [s for s in before_subs
                      if _norm_lang(s.get("language", "und")) not in KEEP_SUB_LANGS]

    print(f"\n  {C}⚡ Auto-Fix v5{X}  {D}Engine: {engine}{X}")
    if dropped_audio:
        print(f"  {Y}🗑 Dropping audio langs: "
              f"{', '.join(t['language'] for t in dropped_audio)}{X}")
    if sorted_audio:
        print(f"  {G}✔ Audio order → "
              f"[{'] ['.join(t['language'] for t in sorted_audio)}]{X}")
    else:
        print(f"  {R}⚠ No matching audio tracks found!{X}")
    if dropped_subs:
        print(f"  {Y}🗑 Dropping sub langs: "
              f"{', '.join(s['language'] for s in dropped_subs)}{X}")
    if eng_subs:
        _kept_sub_display = sorted(KEEP_SUB_LANGS, key=_lang_pri)
        print(f"  {G}✔ {len(eng_subs)} subtitle(s) kept "
              f"({', '.join(_kept_sub_display)}){X}")
    else:
        print(f"  {D}  no matching subtitles{X}")
    print()
...
def manual_edit_and_upload():
    ...
    for i, raw in enumerate(raw_urls):
        ...
        filtered_audio = [t for t in a_tracks
                          if _norm_lang(t["language"]) in KEEP_AUDIO_LANGS]
        sorted_audio   = sorted(filtered_audio, key=lambda t: _lang_pri(t["language"]))
        all_subs       = sorted(
                            [s for s in s_tracks
        ...
        dropped_a = [t for t in a_tracks if _norm_lang(t["language"]) not in KEEP_AUDIO_LANGS]
        dropped_s = [s for s in s_tracks if _norm_lang(s.get("language", "und")) not in KEEP_SUB_LANGS]

        print(f"\n  {C}File:{X} {Y}{local_fp.name[:60]}{X}")
        print(f"  Audio: {len(a_tracks)} track(s) → keeping {len(sorted_audio)}  "
              f"Subs: {len(s_tracks)} track(s) → keeping {len(all_subs)}")
        if dropped_a:
            print(f"  {Y}🗑 Dropping audio: "
                  f"{', '.join(t['language'] for t in dropped_a)}{X}")
        if dropped_s:
            print(f"  {Y}🗑 Dropping subs:  "
                  f"{', '.join(s['language'] for s in dropped_s)}{X}")
        ...
        # Keep only KEEP_SUB_LANGS subtitle tracks
        eng_subs = all_subs
...
print(f"{G}✔ Tamizhan v9.0 loaded — all 6 menus ready.  ⚡ Smart-skip  🔁 Resume{X}")
main_menu()
