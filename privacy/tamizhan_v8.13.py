# ╔══════════════════════════════════════════════════════════════════════╗
# ║   TAMIZHAN MOVIES — ULTRA DOWNLOADER + AUTO-FIX  v8.13               ║
# ║                                                                       ║
...
# ║                                                                       ║
# ║  v8.12: Single shared dashboard — one \r line shows all active       ║
# ║         files compactly; each file gets a clean permanent summary     ║
# ║         when done.  No scroll, no overwrite, works in Colab.          ║
# ║         Connection pool 256. urllib3 warnings suppressed.             ║
# ╚══════════════════════════════════════════════════════════════════════╝
...
# ══════════════════════════════════════════════════════════════════════
#  COLAB PROGRESS DISPLAY  v8.12
#
#  KEY INSIGHT: In Colab's output cell, \r only rewrites the LAST line
#  ever printed.  The moment any other print() happens, that line is
#  gone forever.  Multiple independent \r bars are impossible.
#
#  SOLUTION — Single shared dashboard:
#    • One global ProgressDashboard (singleton) owns ONE \r line.
#    • Every LiveBar registers itself; the dashboard's background
#      thread rewrites that single line every second with a compact
#      summary of ALL active bars.
#    • When a bar finishes it prints ONE permanent \n summary line,
#      then the dashboard continues with the remaining bars.
#    • All other log messages (remux, upload, errors) are queued while
#      the dashboard is active and flushed after the last bar finishes.
#    • Refresh rate: 1 Hz — slow enough to never flood Colab's buffer.
# ══════════════════════════════════════════════════════════════════════

_DASH_LOCK  = threading.Lock()   # serialises all stdout writes
_LOG_QUEUE      = []
_LOG_QUEUE_LOCK = threading.Lock()


def _colab_log(msg):
    """Queue log line while dashboard is running; else print immediately."""
    if ProgressDashboard.get().is_active():
...
class ProgressDashboard:
    """
    Singleton — one \r line showing all active downloads.

    Layout (one file per sub-line, all on the same \r line, separated by │):
      ↓ File1  45%  38 MB/s │ ↓ File2  12%  41 MB/s │ ⚙ File3 remux 30%
    """
    _inst     = None
    _inst_lk  = threading.Lock()
    ...
    def unregister(self, bar, summary_line):
        """Remove bar, print its one-line summary permanently, flush logs if last."""
        with self._lock:
            if bar in self._bars:
                self._bars.remove(bar)
            remaining = len(self._bars)

        # Erase the \r dashboard line, print the summary, restart dashboard
        with _DASH_LOCK:
            print(f"\r{' ' * 120}\r{summary_line}")
    ...
    def _draw(self):
        ...
        for bar in bars:
            with bar._lock:
                pct    = min(bar.done / bar.total, 1.0)
                filled = int(BAR_W * pct)
                spd    = bar._spd
                ela    = int(time.time() - bar._t0)
                label  = bar.label[:24]
                phase  = bar.phase if bar.phase else "↓ Download"
                spd_s  = f"{spd/1_048_576:6.1f} MB/s" if spd > 0 else "  ?.? MB/s"
                eta    = (bar.total - bar.done) / spd if spd > 0 else 0
                done_s = _fmt_size(bar.done)
                tot_s  = _fmt_size(bar.total)
            bar_str = f"{G}{'▰' * filled}{D}{'▱' * (BAR_W - filled)}{X}"
            lines.append(
                f"  {C}{label:<24}{X} {D}{phase}{X} [{bar_str}]"
                f" {G}{pct*100:5.1f}%{X}"
                f"  {done_s}/{tot_s}"
                f"  {Y}{spd_s}{X}"
                f"  ETA {_fmt_eta(eta)}"
...
# Alias so old call sites (MultiBarManager.get().log(...)) still work
MultiBarManager = ProgressDashboard


class LiveBar:
    """
    Progress bar that feeds into the shared ProgressDashboard.

    Usage is identical to the old LiveBar — create, call update(),
    call finish().  The dashboard handles all screen output.
    """
    BAR_WIDTH   = 32   # kept for _render_line compat; not used for display
    ...
    def __init__(self, total_bytes, label="", phase=""):
        ...
        self._lt    = time.time()
        self._spd   = 0.0
        ProgressDashboard.get().register(self)
    ...
    def finish(self, ok=True):
        ...
        bar_str = f"{G}{'▰' * BAR_W}{X}"
        summary = (
            f"  {C}{self.label:<24}{X}{ph} [{bar_str}]"
            f" {G}100.0%{X}"
            f"  {_fmt_size(self.total)}"
            f"  avg {Y}{avg:.1f} MB/s{X}  {int(ela)}s"
            f"  {st}"
        )
        ProgressDashboard.get().unregister(self, summary)
...
def _banner():
    print(f"""{C}
╔══════════════════════════════════════════════════════════════╗
║   🎬📺 TAMIZHAN MOVIES — DOWNLOADER + AUTO-FIX  v8.13       ║
║   ⚡ All 5 menus parallel  [▰▰▰▰▰▱▱▱] live bar display      ║
╚══════════════════════════════════════════════════════════════╝{X}
  Folder  : {Y}{FOLDER}{X}
...
print(f"{G}✔ Tamizhan v8.13 loaded — [▰▰▰▰▱▱▱▱] live bars  ⚡ parallel pipelines  🔌 Pool=256.{X}")
main_menu()
