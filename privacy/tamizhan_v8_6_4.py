# ╔══════════════════════════════════════════════════════════════════════╗
# ║   TAMIZHAN MOVIES — ULTRA DOWNLOADER + AUTO-FIX  v8.6              ║
...
# ══════════════════════════════════════════════════════════════════════
#  GOOGLE AUTH (must run before Drive mount)
# ══════════════════════════════════════════════════════════════════════
try:
...
LANG_ORDER   = ["tam", "tel", "hin", "mal", "kan", "eng", "und"]
THREADS      = 32
STREAM_CONNS = 8
CHUNK_MB     = 16
MAX_RETRY    = 3
...
def _process_video_files_from_dir(extract_dir, folder_name, new_folder_id, tok,
    ...
    for i, vf in enumerate(video_files):
        ...
        eng_s         = [s for s in subs if s["language"].lower().strip() == "eng"]
        other_s       = len(subs) - len(eng_s)
        ...
        sub_col = f"{G}ENG✔{X}" if eng_s else f"{D} — {X}"
        if other_s:
            sub_col += f" {R}-{other_s}sub{X}"
        print(
    ...
    for vi, vf in enumerate(video_files):
        ...
        print(f"  {C}⚡ Remux [{engine}]{X}  Audio: {len(sorted_a)} tracks  "
              f"Top: {C}{top_lang}{X}  ENG subs: {len(eng_s)}")
        if other_s:
            print(f"  {R}✘ {other_s} non-ENG sub(s) will be removed{X}")
...
def auto_fix_v5(fp, known_fid=None, drive_dest=None):
    ...
    eng_subs     = [s for s in before_subs if s["language"].lower().strip() == "eng"]
    other_subs   = len(before_subs) - len(eng_subs)
    ...
    if other_subs:
        print(f"  {R}✘ {other_subs} non-ENG subtitle(s) will be removed{X}")
    if eng_subs:
        print(f"  {G}✔ {len(eng_subs)} ENG subtitle(s) kept{X}")
    else:
...
def manual_edit_and_upload():
    ...
    for i, raw in enumerate(raw_urls):
        ...
        # After editing, keep only ENG subs (matching auto-fix rule)
        eng_subs = [s for s in all_subs if s["language"].lower().strip() == "eng"]
        removed  = len(all_subs) - len(eng_subs)
        if removed:
            print(f"  {R}✘ {removed} non-ENG subtitle(s) will be removed (auto-fix rule){X}")
...
def _banner():
    print(f"""{C}
╔══════════════════════════════════════════════════════════════╗
║    🎬📺 TAMIZHAN MOVIES — DOWNLOADER + AUTO-FIX  v8.6       ║
...
╚══════════════════════════════════════════════════════════════╝{X}
  ...
  Auto-Fix rules:
    ...
    🔊 Audio      → {G}Tamil first{X}, others demoted, titles cleared
    💬 Subtitles  → {G}ENG only{X} kept, all others {R}removed{X}
...
print(f"{G}✔ Tamizhan v8.6 loaded — all 6 menus ready.{X}")
main_menu()