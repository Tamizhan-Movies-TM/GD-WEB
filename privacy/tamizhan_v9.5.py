# ╔══════════════════════════════════════════════════════════════════════╗
# ║   TAMIZHAN MOVIES — ULTRA DOWNLOADER + AUTO-FIX  v9.5                ║
...
# ║   v9.5: ⚡ Smart remux skip  🔁 Resume  🎵 A/V Sync Fix             ║
# ╚══════════════════════════════════════════════════════════════════════╝
...
def _probe_audio_delays(fp):
    ...
    Only uses explicit codec_delay fields — never timestamp deltas.
    Timestamp deltas (minimum_timestamp, start_time) cause false positives
    on legitimately-synced multi-audio files and are intentionally skipped.

    Methods (codec_delay only):
      1. mkvmerge -J  codec_delay  (MKV only — authoritative header field, ns)
      2. ffprobe      codec_delay tag  (fallback for non-MKV or mkvmerge absent)

    A positive value means audio is delayed → mkvmerge --sync shifts it back.
    A negative value means audio is early  → mkvmerge --sync shifts it forward.
    """
    fp = Path(fp)
    delays = {}

    # ── Method 1: mkvmerge -J codec_delay (MKV only, most accurate) ──
    if fp.suffix.lower() == ".mkv" and shutil.which("mkvmerge"):
        try:
            ...
            # Map mkvmerge audio track order → ffprobe stream indices
            ...
            for t in tracks:
                if t.get("type") != "audio":
                    continue
                cd_ns = t.get("properties", {}).get("codec_delay")
            ...
            return delays   # mkvmerge codec_delay is authoritative — stop here

        except Exception:
            pass  # fall through to ffprobe

    # ── Method 2: ffprobe codec_delay tag (non-MKV or mkvmerge absent) ─
    try:
        r = subprocess.run(
            ["ffprobe", "-v", "quiet", "-print_format", "json",
             "-show_streams", str(fp)],
...
def manual_edit_and_upload():
    ...
    for i, raw in enumerate(raw_urls):
        ...
        if auto_delays:
            # Delays detected — show them and let user override
            parts = ", ".join(
            ...
            print(f"  {Y}Auto-detected delays: {parts}{X}")
            print(f"  {W}Enter override correction (e.g. +200 or -150 ms).{X}")
            ...
            # No sync issues detected — skip silently
            print(f"  {G}✔ Audio sync OK — no correction needed.{X}")
...
def _banner():
    print(f"""{C}
╔══════════════════════════════════════════════════════════════╗
║    🎬📺 TAMIZHAN MOVIES — DOWNLOADER + AUTO-FIX  v9.5       ║
...
print(f"{G}✔ Tamizhan v9.5 loaded — all 6 menus ready.  ⚡ Smart-skip  🔁 Resume  🎵 A/V Sync Fix{X}")
main_menu()
