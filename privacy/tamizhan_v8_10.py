# ╔══════════════════════════════════════════════════════════════════════╗
# ║   TAMIZHAN MOVIES — ULTRA DOWNLOADER + AUTO-FIX  v8.10               ║
# ║                                                                       ║
...
# ║                                                                       ║
# ║  v8.10: Multi-bar display — each parallel download gets its own      ║
# ║         stable terminal row; bars never overwrite each other.         ║
# ╚══════════════════════════════════════════════════════════════════════╝
...
# ══════════════════════════════════════════════════════════════════════
#  MULTI-BAR MANAGER + LIVE PROGRESS BAR
#
#  When multiple downloads run simultaneously, a single global
#  MultiBarManager owns all active LiveBar slots.  It redraws every
#  bar on its own fixed terminal line using ANSI cursor movement so
#  bars never overwrite each other.
#
#  Single-bar usage (unchanged):  LiveBar(size, label, phase)
#  Multi-bar usage (automatic):   MultiBarManager registers / removes
#                                  bars transparently when LiveBar
#                                  is created/finished.
# ══════════════════════════════════════════════════════════════════════
class MultiBarManager:
    """
    Global singleton that owns a block of terminal lines, one per
    active LiveBar.  Redraws the entire block at 5 Hz so bars never
    collide even when dozens of threads call bar.update() at once.
    """

    REFRESH_HZ  = 5          # redraws per second
    _instance   = None
    ...
    def __init__(self):
        ...
        self._bars      = []          # list of LiveBar in registration order
        self._n_lines   = 0           # how many lines the block currently occupies
        self._active    = False
        self._thread    = None
        self._log_queue = []          # pending plain-text log lines
    ...
    def unregister(self, bar, summary_line):
        """Remove bar from the live block; print its summary as a plain log line."""
        with self._lock:
            # Queue the summary to print above the remaining live block
            self._log_queue.append(summary_line)
            if bar in self._bars:
                self._bars.remove(bar)
            if not self._bars:
                self._active = False

    # ── Log lines printed above the live block ───────────────────────
    def log(self, line):
        """Print a plain log line above the live bar block (thread-safe)."""
        with self._lock:
    ...
    def _loop(self):
        ...
        while self._active:
            ...
            self._redraw()
        # Final redraw to flush last state
        self._redraw(final=True)

    def _redraw(self, final=False):
        import sys
        with self._lock:
            log_lines = self._log_queue[:]
            self._log_queue.clear()
            bars      = list(self._bars)

        # Move cursor up to overwrite the previous block
        if self._n_lines > 0:
            sys.stdout.write(f"\033[{self._n_lines}A")   # cursor up N lines
            sys.stdout.write("\033[J")                    # erase from cursor down

        # Print any queued log lines (they stay permanently)
        for ll in log_lines:
            sys.stdout.write(ll + "\n")
        ...
        rendered = 0
        for bar in bars:
            line = bar._render_line()
            sys.stdout.write(line + "\n")
            rendered += 1

        sys.stdout.flush()
        self._n_lines = rendered   # remember how many lines we drew


class LiveBar:
    """
    Thread-safe animated progress bar.

    If more than one LiveBar is alive at the same time the
    MultiBarManager takes over rendering so each bar occupies its
    own stable terminal line.  Single-bar behaviour is unchanged.
    """
    ...
    def __init__(self, total_bytes, label="", phase=""):
        ...
        self._spd    = 0.0
        self._alive  = True
        # Register with the global multi-bar manager
        MultiBarManager.get().register(self)
    ...
    # ── Render one bar line (called by MultiBarManager) ──────────────
    ...
    def finish(self, ok=True):
        ...
        ph  = f" {D}{self.phase}{X}" if self.phase else ""
        st  = f"{G}✔{X}" if ok else f"{R}✘{X}"
        summary = (
            f"  {C}{self.label:<28}{X}{ph}  {st}  "
            f"{_fmt_size(self.total)}  avg {avg:.1f} MB/s  {int(ela)}s"
        )
        MultiBarManager.get().unregister(self, summary)
        time.sleep(0.1)   # give manager one tick to flush the summary

    # ── Compat shims used by the old ZIP downloader code ─────────────
    # (The old code directly poked _alive / _lb / _spd / _t0 / _lt
    #  when retrying after a failed parallel attempt — keep working.)
    def _restart_phase(self, new_phase):
        """Reset progress counters for a retry attempt."""
        with self._lock:
...
def _banner():
    print(f"""{C}
╔══════════════════════════════════════════════════════════════╗
║   🎬📺 TAMIZHAN MOVIES — DOWNLOADER + AUTO-FIX  v8.10       ║
║   ⚡ All 5 menus parallel  📊 Clean multi-bar progress       ║
╚══════════════════════════════════════════════════════════════╝{X}
  Folder  : {Y}{FOLDER}{X}
...
print(f"{G}✔ Tamizhan v8.10 loaded — all 6 menus ready.  ⚡ Menus [1-5] parallel  📊 clean multi-bar display.{X}")
main_menu()
