# ╔══════════════════════════════════════════════════════════════════════╗
# ║   TAMIZHAN MOVIES — ULTRA DOWNLOADER + AUTO-FIX  v9.2                ║
...
# ║   v9.2: ⚡ Smart remux skip  🔁 Resume  🎵 A/V Sync Fix             ║
# ╚══════════════════════════════════════════════════════════════════════╝
...
def _run_remux_with_titles(src, tmp, sorted_audio, eng_subs, engine,
    ...
    def _aud_title(t):
        """User title if non-empty, otherwise the language display name."""
        return t.get("title") or _lang_display(t["language"])

    def _sub_title(s):
        """User title if non-empty, otherwise the language display name."""
        return s.get("title") or _lang_display(s.get("language", "und"))
    ...
    try:
        ...
        # mkvpropedit pass — writes language display names as track titles
        if engine == "mkvmerge":
            if not shutil.which("mkvpropedit"):
                subprocess.run(
                    ["apt-get", "install", "-y", "-q", "mkvtoolnix"],
...
def _banner():
    print(f"""{C}
╔══════════════════════════════════════════════════════════════╗
║    🎬📺 TAMIZHAN MOVIES — DOWNLOADER + AUTO-FIX  v9.2       ║
...
print(f"{G}✔ Tamizhan v9.2 loaded — all 6 menus ready.  ⚡ Smart-skip  🔁 Resume  🎵 A/V Sync Fix{X}")
main_menu()
