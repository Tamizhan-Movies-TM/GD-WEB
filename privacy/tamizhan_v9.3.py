# ╔══════════════════════════════════════════════════════════════════════╗
# ║   TAMIZHAN MOVIES — ULTRA DOWNLOADER + AUTO-FIX  v9.3                ║
...
# ║   v9.3: ⚡ Smart remux skip  🔁 Resume  🎵 A/V Sync Fix             ║
# ╚══════════════════════════════════════════════════════════════════════╝
...
def _probe_audio_delays(fp):
    ...
    Strategy (in priority order):
      1. MKV CodecDelay tag  — most accurate for Opus/AAC/EAC3 in MKV
      2. per-stream start_time minus video start_time
      3. 0 ms (no detectable delay)

    A positive value means the audio starts AFTER the video (add delay).
    A negative value means the audio starts BEFORE the video (subtract delay).
    """
    delays = {}
    try:
        r = subprocess.run(
    ...
    for s in streams:
        ...
        # 1. CodecDelay (MKV only) — stored in nanoseconds in ffprobe tags
        cd_ns = None
        ...
        if cd_ns is not None:
            delays[idx] = round(cd_ns / 1_000_000)   # ns → ms
            continue

        # 2. start_time delta vs video
        try:
...
def _banner():
    print(f"""{C}
╔══════════════════════════════════════════════════════════════╗
║    🎬📺 TAMIZHAN MOVIES — DOWNLOADER + AUTO-FIX  v9.3       ║
...
print(f"{G}✔ Tamizhan v9.3 loaded — all 6 menus ready.  ⚡ Smart-skip  🔁 Resume  🎵 A/V Sync Fix{X}")
main_menu()
