...
# ║   v9.3: ⚡ Fast AAC coder  💬 Sub default=False fix  🔁 Resume      ║
# ╚══════════════════════════════════════════════════════════════════════╝
...
# Codecs that must be transcoded to AAC (not MKV-friendly or widely unsupported)
_TRANSCODE_CODECS = {"eac3", "ac3", "truehd", "dts"}


def _has_transcode_audio(audio_tracks):
    """Return True if any audio track has a codec that needs transcoding to AAC."""
...
def _build_remux_cmd(src, tmp, sorted_audio, eng_subs, engine):
    ...
    if engine == "mkvmerge":
        ...
        meta_args = []
        disp_args = []
        codec_args = []  # per-stream codec overrides
        ...
        # ── Speed: fast AAC coder if any track needs transcoding ──────
        has_aac_transcode = any(
            t.get("codec", "").lower() in _TRANSCODE_CODECS for t in sorted_audio
        )
        aac_speed_args = ["-aac_coder", "fast"] if has_aac_transcode else []

        return [
            "ffmpeg", "-y", "-loglevel", "error",
            "-threads", "0",               # use all CPU cores for decoding
            "-filter_threads", "0",        # use all CPU cores for audio filter graph
            "-progress", "pipe:1", "-stats_period", "1",
            "-i", str(src),
            *map_args,
            "-c:v", "copy",        # video — never re-encode
            "-c:s", "copy",        # subtitles — copy as-is
            *codec_args,           # per-audio-stream codec (aac or copy)
            *aac_speed_args,       # ⚡ fast AAC encoding when transcoding
...
def _tracks_need_fix(fp, sorted_audio, eng_subs):
    ...
    # ── 0. Check for codecs that require transcoding (e.g. E-AC3 → AAC) ─
    for t in a_tracks:
        codec = t.get("codec", "").lower()
        if codec in _TRANSCODE_CODECS:
            return True, f"audio codec {codec!r} needs transcoding to AAC"
    ...
    # ── 4b. Subtitle flag-default must always be False (no auto-display) ─
    for i, on_disk in enumerate(s_tracks):
        if bool(on_disk.get("is_default", False)):
            return True, f"sub[{i}] flag-default is True — must be False (no auto-display)"
...
def _process_video_files_from_dir(extract_dir, folder_name, new_folder_id, tok,
    ...
    for i, vf in enumerate(video_files):
        ...
        engine        = ("ff→aac" if _has_transcode_audio(aud)
                         else ("mkv" if vf.suffix.lower() == ".mkv" else "ff"))
    ...
    for vi, vf in enumerate(video_files):
        ...
        engine   = ("ffmpeg" if _has_transcode_audio(aud)
                    else ("mkvmerge" if vf.suffix.lower() == ".mkv" else "ffmpeg"))
        top_lang = sorted_a[0]["language"]
        transcode_codecs = {t.get("codec","").upper() for t in aud
                            if t.get("codec","").lower() in _TRANSCODE_CODECS}
        transcode_note = (f"  {Y}🔄 {', '.join(sorted(transcode_codecs))}→AAC{X}"
                          if transcode_codecs else "")
...
def auto_fix_v5(fp, known_fid=None, drive_dest=None):
    ...
    need_transcode = _has_transcode_audio(before_audio)
    # mkvmerge cannot transcode — fall back to ffmpeg whenever codec conversion is needed
    engine       = "ffmpeg" if need_transcode else (
                   "mkvmerge" if fp.suffix.lower() == ".mkv" else "ffmpeg")
    ...
    if need_transcode:
        codecs_found = {t.get("codec","?").upper() for t in before_audio
                        if t.get("codec","").lower() in _TRANSCODE_CODECS}
        print(f"  {Y}🔄 Audio transcode: {', '.join(sorted(codecs_found))} → AAC"
              f"  {D}(video untouched — fast){X}")
...
def manual_edit_and_upload():
    ...
    for i, raw in enumerate(raw_urls):
        ...
        # ── Remux ─────────────────────────────────────────────────────
        need_transcode = _has_transcode_audio(sorted_audio)
        engine = ("ffmpeg" if need_transcode
                  else ("mkvmerge" if local_fp.suffix.lower() == ".mkv" else "ffmpeg"))
        tmp    = Path("/content") / (local_fp.stem[:60] + "_MANU6_TMP" + local_fp.suffix)
        if need_transcode:
            tc_codecs = {t.get("codec","").upper() for t in sorted_audio
                         if t.get("codec","").lower() in _TRANSCODE_CODECS}
            print(f"\n  {Y}🔄 Audio transcode: {', '.join(sorted(tc_codecs))} → AAC"
                  f"  {D}(video untouched){X}")
        print(f"\n  {C}⚙ Remuxing  [{engine}]...{X}")
...
print(f"{G}✔ Tamizhan v9.3 loaded — all 6 menus ready.  ⚡ Fast-AAC  💬 Sub=False  🔁 Resume  🔊 E-AC3→AAC{X}")
main_menu()
