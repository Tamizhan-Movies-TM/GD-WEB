...
def _build_remux_cmd(src, tmp, sorted_audio, eng_subs, engine,
    ...
    manual_delays: not used in Menus 1-5 (sync correction is Menu 6 only).
    """
    delays = {}
...
def _run_remux_with_titles(src, tmp, sorted_audio, eng_subs, engine,
    ...
    manual_delays: dict {stream_index: delay_ms} from Menu 6 user input.
    """
    src   = Path(src)
    tmp   = Path(tmp)
    fsize = src.stat().st_size
    ...
    # Use only user-supplied corrections (no auto-detect in Menu 6 base either)
    delays = dict(manual_delays or {})
    if delays:
        parts = ", ".join(f"idx {k}: {v:+d} ms" for k, v in sorted(delays.items()))
        print(f"  {D}⏱ Audio sync offsets applied: {parts}{X}")
...
def manual_edit_and_upload():
    ...
    for i, raw in enumerate(raw_urls):
        ...
        # ── A/V sync correction (Menu 6 only) ────────────────────────
        auto_delays  = _probe_audio_delays(local_fp)
        # Start with auto as base; user overrides replace individual tracks
        manual_delays: dict = dict(auto_delays)
