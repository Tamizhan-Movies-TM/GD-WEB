# ╔══════════════════════════════════════════════════════════════════════╗
# ║   TAMIZHAN MOVIES — ULTRA DOWNLOADER + AUTO-FIX  v9.4                ║
...
# ║   v9.4: ⚡ Smart remux skip  🔁 Resume  🎵 A/V Sync Fix             ║
# ╚══════════════════════════════════════════════════════════════════════╝
...
LANG_ORDER   = ["tam", "tel", "mal", "kan", "hin", "eng", "kor", "und"]
THREADS      = 32
STREAM_CONNS = 8
CHUNK_MB     = 16
MAX_RETRY    = 3
...
# Languages we keep in BOTH audio-sort and subtitle tracks.
# Subtitle tracks whose language is NOT in this set are dropped.
_KEEP_LANGS = {"tam", "tel", "mal", "kan", "hin", "eng", "kor"}


def _filter_subs(subs):
    """Keep only subtitle tracks whose language is in the priority set."""
    return [s for s in subs if s.get("language", "und") in _KEEP_LANGS]


# Human-readable display names for track title metadata.
# Media players show the track's title tag; using a proper name avoids
# players falling back to "Track N" when the tag is blank.
_LANG_DISPLAY = {
    "tam": "Tamil",
    "tel": "Telugu",
    "mal": "Malayalam",
...
def _lang_display(lang_code):
    """Return a human-readable name for a 3-letter ISO language code."""
    return _LANG_DISPLAY.get(lang_code.lower().strip(), lang_code.upper())


def _prompt_und_langs(audio_tracks):
    """
    For any audio track whose language is 'und', ask the user to enter
    the correct 3-letter ISO code (e.g. tam, tel, hin).
    Modifies the track dicts in-place; returns True if any were changed.
    """
    und_tracks = [t for t in audio_tracks if t.get("language", "und") == "und"]
    if not und_tracks:
        return False

    print(f"\n  {Y}⚠ {len(und_tracks)} audio track(s) with unknown language (und):{X}")
    changed = False
    for t in und_tracks:
        idx   = t.get("idx", "?")
        title = t.get("title") or f"Track idx {idx}"
        print(f"  {C}Track idx {idx}{X}  title: {Y}{title}{X}")
        while True:
            try:
                raw = input(
                    f"    Enter language code (tam/tel/mal/kan/hin/eng/kor/…)"
                    f" or blank to keep 'und': "
                ).strip().lower()
            except (EOFError, KeyboardInterrupt):
                raw = ""
            if not raw:
                print(f"    {D}Kept as 'und'{X}")
                break
            if len(raw) == 3 and raw.isalpha():
                t["language"] = raw
                print(f"    {G}✔ Set to '{raw}' ({_lang_display(raw)}){X}")
                changed = True
                break
            print(f"    {R}Invalid — must be a 3-letter code (e.g. tam, tel, hin){X}")
    return changed
...
def _probe_audio_delays(fp):
    """
    Return a dict mapping stream index → delay_ms (int, may be negative).

    Strategy (in priority order — most accurate first):
      1. mkvmerge -J  codec_delay       (MKV only — raw header, nanoseconds)
      2. mkvmerge -J  minimum_timestamp (MKV only — actual first PTS per track)
      3. ffprobe      codec_delay tag   (MKV fallback via ffprobe tags)
      4. ffprobe      start_time delta  (all formats fallback)

    A positive value means audio starts AFTER video → shift audio earlier.
    A negative value means audio starts BEFORE video → shift audio later.
    """
    fp = Path(fp)
    delays = {}

    # ── Method 1 & 2: mkvmerge -J (MKV only, most accurate) ──────────
    if fp.suffix.lower() == ".mkv" and shutil.which("mkvmerge"):
        try:
            r = subprocess.run(
                ["mkvmerge", "-J", str(fp)],
                capture_output=True, text=True, timeout=60,
            ...
            # Build video minimum_timestamp as reference (in ns)
            v_min_ts_ns = 0
            for t in tracks:
                if t.get("type") == "video":
                    try:
                        v_min_ts_ns = int(
                            t.get("properties", {}).get("minimum_timestamp", 0) or 0
                        )
                    except Exception:
                        pass
                    break

            # Map mkvmerge track id → ffprobe stream index via codec type order
            # mkvmerge uses its own track numbering; we match by audio track order
            ...
            try:
                ...
                ff_streams = json.loads(rp.stdout).get("streams", [])
                audio_stream_indices = [
                    s["index"] for s in ff_streams
            ...
            for t in tracks:
                ...
                # Method 1: codec_delay (ns) — highest priority
                cd_ns = props.get("codec_delay")
                if cd_ns is not None:
                    try:
                        d_ms = round(int(cd_ns) / 1_000_000)
                        if mkv_audio_idx < len(audio_stream_indices):
                            stream_idx = audio_stream_indices[mkv_audio_idx]
                            if d_ms != 0:
                                delays[stream_idx] = d_ms
                        mkv_audio_idx += 1
                        continue
                    except Exception:
                        pass

                # Method 2: minimum_timestamp delta vs video (ns)
                min_ts = props.get("minimum_timestamp")
                if min_ts is not None:
                    try:
                        a_min_ts_ns = int(min_ts)
                        d_ms = round((a_min_ts_ns - v_min_ts_ns) / 1_000_000)
                        if mkv_audio_idx < len(audio_stream_indices):
                            stream_idx = audio_stream_indices[mkv_audio_idx]
                            if d_ms != 0:
                                delays[stream_idx] = d_ms
            ...
            if delays:
                return delays   # mkvmerge data is authoritative — return early

        except Exception:
            pass  # fall through to ffprobe methods

    # ── Methods 3 & 4: ffprobe (all formats) ─────────────────────────
    try:
        r = subprocess.run(
            ["ffprobe", "-v", "quiet", "-print_format", "json",
             "-show_streams", str(fp)],
    ...
    # Video reference start_time
    v_start = 0.0
    for s in streams:
        if s.get("codec_type") == "video":
            try:
                v_start = float(s.get("start_time", 0) or 0)
            except Exception:
                pass
            break
    ...
    for s in streams:
        ...
        idx = s.get("index")
        if idx in delays:
            continue   # already resolved by mkvmerge

        # Method 3: codec_delay tag via ffprobe
        cd_ns = None
        for tag_key in ("codec_delay", "CODEC_DELAY"):
            raw = s.get("tags", {}).get(tag_key) or s.get(tag_key)
            if raw is not None:
                try:
                    cd_ns = int(raw)
                    break
                    ...
                    pass
        if cd_ns is not None:
            d_ms = round(cd_ns / 1_000_000)
            if d_ms != 0:
                delays[idx] = d_ms
            continue

        # Method 4: start_time delta vs video
        try:
            a_start  = float(s.get("start_time", 0) or 0)
            delta_ms = round((a_start - v_start) * 1000)
            if delta_ms != 0:
                delays[idx] = delta_ms
        except Exception:
            pass
...
def _audio_tracks(fp):
    ...
    for i, s in enumerate(_probe(fp, "a")):
        tags = s.get("tags", {})
        # Use only the explicit title tag (blank = no title written).
        # handler_name is an internal codec field, not a user-visible title.
        out.append({
            "idx":        s.get("index", i),
            "track_num":  i,
            "title":      tags.get("title") or "",   # blank = no title tag set
...
def _build_remux_cmd(src, tmp, sorted_audio, eng_subs, engine,
    ...
    Subtitle tracks outside the keep-list are already filtered before this call.
    Track titles are cleared (blank) in auto-fix mode.

    manual_delays: optional dict {stream_index: delay_ms} — values from
    _probe_audio_delays() or user-entered corrections.  Applied as
    --sync (mkvmerge) or -af adelay (ffmpeg).
    """
    # Auto-detect per-track delays from source file to preserve sync
    auto_delays = _probe_audio_delays(src)
    # manual_delays override auto-detected ones (user correction wins)
    delays = {**auto_delays, **(manual_delays or {})}

    if delays:
        parts = ", ".join(f"idx {k}: {v:+d} ms" for k, v in sorted(delays.items()))
        print(f"  {D}⏱ Audio delays detected: {parts}{X}")
    ...
    if engine == "mkvmerge":
        ...
        for i, t in enumerate(sorted_audio):
            ...
            # Preserve detected/corrected sync offset
            d = delays.get(t["idx"])
            if d is not None and d != 0:
                audio_args += ["--sync", f"{sid}:{d}"]
        ...
        # ffmpeg: apply per-track delay via -itsoffset before each input map.
        # For copy-mode we inject a filter only when a non-zero delay exists;
        # zero-delay tracks are passed through unchanged.
        delay_args = []
        for i, t in enumerate(sorted_audio):
            d = delays.get(t["idx"])
            if d and d != 0:
                sec = d / 1000.0
                delay_args += [f"-af:a:{i}", f"adelay={max(d,0)}|{max(d,0)},atrim=start={max(-d/1000,0)}"]
...
# ══════════════════════════════════════════════════════════════════════
#  SMART REMUX SKIP  (v9.0)
# ══════════════════════════════════════════════════════════════════════
def _tracks_need_fix(fp, sorted_audio, eng_subs):
    """
    Return (needs_fix: bool, reason: str).

    Full checklist — every condition must be true to skip remux:
      1.  Audio count matches expected
      2.  Audio language order correct (highest-priority first)
    ...
    # ── 8-11. Per-subtitle-track checks ──────────────────────────────
    for i, (on_disk, expected) in enumerate(zip(s_tracks, eng_subs)):
        exp_lang = _norm_lang(expected.get("language", "und"))
        # 8. Language tag
        if _norm_lang(on_disk["language"]) != exp_lang:
            return True, (f"sub[{i}] lang {on_disk['language']!r} "
                          f"≠ expected {exp_lang!r}")
        # 9. Default flag must be False
        if bool(on_disk.get("is_default", False)):
            return True, f"sub[{i}] default flag should be False"
    ...
    try:
        r = subprocess.run(
            ["ffprobe", "-v", "quiet", "-print_format", "json",
             "-show_format", "-show_streams", "-select_streams", "v", str(fp)],
            capture_output=True, text=True, timeout=30,
        )
        data = json.loads(r.stdout)
        # 12. Video track name must be blank
        for vs in data.get("streams", []):
            vtitle = (vs.get("tags", {}).get("title") or "").strip()
            if vtitle:
                return True, f"video track name not blank: {vtitle!r}"
        # 13. Container title must be blank
        fmt_title = (data.get("format", {}).get("tags", {}).get("title") or "").strip()
        if fmt_title:
...
def _run_remux_with_titles(src, tmp, sorted_audio, eng_subs, engine,
                           manual_delays=None):
    """
    Like _run_remux but writes clean language-display names as track titles.
    If the user typed a custom title in Menu [6] it is kept; otherwise the
    title is set to the language display name (e.g. "Tamil", "Telugu")
    so media players show the language name rather than "Track N".
    Used exclusively by the Manual Edit menu (option [6]).

    manual_delays: optional dict {stream_index: delay_ms} to correct A/V sync.
    """
    src   = Path(src)
    tmp   = Path(tmp)
    fsize = src.stat().st_size
    ...
    # Merge auto-detected delays with user-supplied corrections
    auto_delays = _probe_audio_delays(src)
    delays = {**auto_delays, **(manual_delays or {})}
    if delays:
        parts = ", ".join(f"idx {k}: {v:+d} ms" for k, v in sorted(delays.items()))
        print(f"  {D}⏱ Audio sync offsets: {parts}{X}")
    ...
    if engine == "mkvmerge":
        ...
        for i, t in enumerate(sorted_audio):
            ...
            audio_args += [
                "--default-track-flag", f"{sid}:{'yes' if i == 0 else 'no'}",
                "--forced-display-flag", f"{sid}:{'yes' if i == 0 else 'no'}",
                "--track-name",          f"{sid}:",   # blank — player reads language code
                "--language",            f"{sid}:{t['language']}",
            ]
            # Apply A/V sync correction
            d = delays.get(t["idx"])
            if d is not None and d != 0:
                audio_args += ["--sync", f"{sid}:{d}"]
        ...
        for i, t in enumerate(sorted_audio):
            map_args  += ["-map", f"0:a:{t['track_num']}"]
            meta_args += [
                f"-metadata:s:a:{i}", "title=",           # blank — player reads language code
                f"-metadata:s:a:{i}", f"language={t['language']}",
            ]
            disp_args += [f"-disposition:a:{i}", "default+forced" if i == 0 else "0"]
        ...
        for i, s in enumerate(eng_subs):
            ...
            slang = s.get("language") or "eng"
            meta_args += [
                f"-metadata:s:s:{i}", "title=",           # blank — player reads language code
        ...
        # ffmpeg A/V sync: apply adelay filter for non-zero offsets
        delay_args = []
        for i, t in enumerate(sorted_audio):
            d = delays.get(t["idx"])
            if d and d != 0:
    ...
    try:
        ...
        # mkvpropedit pass — clears titles, sets language codes + flags
        if engine == "mkvmerge":
            if not shutil.which("mkvpropedit"):
                subprocess.run(
                    ["apt-get", "install", "-y", "-q", "mkvtoolnix"],
...
def _drive_download_local(fid, fname, size):
    ...
    # ── v9.0: check for resumable .part file ─────────────────────────
...
def _process_video_files_from_dir(extract_dir, folder_name, new_folder_id, tok,
    ...
    for i, vf in enumerate(video_files):
        ...
        eng_s         = _filter_subs(subs)   # keep only tam/tel/mal/kan/hin/eng/kor
    ...
    for vi, vf in enumerate(video_files):
        ...
        # If any audio track is 'und', ask once then continue automatically
        if _prompt_und_langs(aud):
            sorted_a = sorted(aud, key=lambda t: _lang_pri(t["language"]))
        ...
        print(f"  {C}⚙ Remux [{engine}]{X}  Audio: {len(sorted_a)} tracks  "
              f"Top: {C}{top_lang}{X}  Subs: {len(eng_s)} (tam/tel/mal/kan/hin/eng/kor)")
...
def auto_fix_v5(fp, known_fid=None, drive_dest=None):
    ...
    # If any audio track is 'und', ask once then continue automatically
    _prompt_und_langs(before_audio)

    sorted_audio = sorted(before_audio, key=lambda t: _lang_pri(t["language"]))
    eng_subs     = _filter_subs(before_subs)   # keep only tam/tel/mal/kan/hin/eng/kor
    dropped_subs = len(before_subs) - len(eng_subs)
    ...
    if eng_subs:
        drop_note = f"  {D}({dropped_subs} other-lang sub(s) removed){X}" if dropped_subs else ""
        print(f"  {G}✔ {len(eng_subs)} subtitle(s) kept (tam/tel/mal/kan/hin/eng/kor){X}{drop_note}")
    else:
        note = f"  {D}({dropped_subs} other-lang sub(s) removed){X}" if dropped_subs else ""
        print(f"  {D}  no subtitles{X}{note}")
    print()

    # ── v9.0: Smart remux skip ────────────────────────────────────────
...
def _manual_edit_all_tracks(audio_tracks, sub_tracks):
    ...
    # Display current state — show effective player name when title is blank
    print(f"\n  {C}🔊 Audio Tracks:{X}")
    for t in audio_tracks:
        dflt      = f"{G}★ default{X}" if t["is_default"] else f"{D}no{X}"
        raw_title = t.get("title") or ""
        show      = raw_title or f"{D}(→ will use: {_lang_display(t['language'])}){X}"
        print(
            f"    [{t['track_num']}] Lang:{C}{t['language']:<5}{X}  "
            f"Title:{Y}{raw_title[:35]:<35}{X}  {show}  "
    ...
    if sub_tracks:
        ...
        for s in sub_tracks:
            ...
            show      = raw_title or f"{D}(→ will use: {_lang_display(s['language'])}){X}"
            print(
    ...
    # Re-sort by priority after all edits so the summary reflects final order
    new_audio = sorted(new_audio, key=lambda t: _lang_pri(t["language"]))
    new_subs  = sorted(new_subs,  key=lambda s: _lang_pri(s.get("language", "und")))

    # Summary — shown in priority order
    print(f"\n{G}{'─' * 68}{X}")
    print(f"  {G}✔ Updated track list (sorted by priority):{X}")
    print(f"  {D}  Player will show these names:{X}")
    for i, t in enumerate(new_audio):
        marker    = f" {G}★ default+forced{X}" if i == 0 else ""
        disp_name = t.get("title") or _lang_display(t["language"])
        print(f"    {G}Audio [{i}]{X}  "
              f"lang:{C}{t['language']:<5}{X}  "
              f"player-name:{Y}{disp_name}{X}{marker}")
    filtered_subs = _filter_subs(new_subs)
    dropped = [s for s in new_subs if s not in filtered_subs]
    for i, s in enumerate(filtered_subs):
        disp_name = s.get("title") or _lang_display(s.get("language", "und"))
        print(f"    {C}Sub   [{i}]{X}  "
              f"lang:{C}{s.get('language', 'und'):<5}{X}  "
              f"player-name:{Y}{disp_name}{X}")
    if dropped:
        print(f"    {Y}⚠ {len(dropped)} subtitle(s) outside tam/tel/mal/kan/hin/eng/kor "
              f"will be dropped: {', '.join(s['language'] for s in dropped)}{X}")
    print(f"{G}{'─' * 68}{X}\n")
...
def gdrive_clone_to_colab():
    ...
    print(f"\n{M}{'═' * 68}{X}")
    print(f"  {M}📂 [4] GDRIVE CLONE → REMUX → PATCH BACK TO DRIVE  v9.0{X}")
...
def manual_edit_and_upload():
    ...
    for i, raw in enumerate(raw_urls):
        ...
        # ── Manual edit ───────────────────────────────────────────────
        # NOTE: _manual_edit_all_tracks already re-sorts new_audio by priority
        # and shows the final order in the summary before returning.
        ...
        # Keep only tam/tel/mal/kan/hin/eng/kor subtitles; drop the rest
        eng_subs = _filter_subs(all_subs)

        # ── A/V sync correction ───────────────────────────────────────
        # Probe auto-delays first so user sees what is already detected
        auto_delays = _probe_audio_delays(local_fp)
        manual_delays: dict = {}

        print(f"\n  {C}⏱ Audio/Video Sync Correction{X}")
        if auto_delays:
            parts = ", ".join(
                f"[{t['language']}] idx {t['idx']}: {auto_delays[t['idx']]:+d} ms"
                for t in sorted_audio if t["idx"] in auto_delays
            )
            print(f"  {D}Auto-detected delays: {parts}{X}")
        else:
            print(f"  {D}No sync issues auto-detected.{X}")

        print(f"  {W}Enter per-track delay corrections (e.g. +200 or -150 ms).{X}")
        print(f"  {D}Leave blank = keep auto value. Enter 0 = force no delay.{X}")
        for t in sorted_audio:
            lang_name = _lang_display(t["language"])
            auto_val  = auto_delays.get(t["idx"], 0)
            prompt_str = (
                f"  {Y}{lang_name}{X} (idx {t['idx']}, auto={auto_val:+d} ms) "
                f"correction [{D}blank=skip{X}]: "
            )
            try:
                raw_d = input(prompt_str).strip()
            except (EOFError, KeyboardInterrupt):
                raw_d = ""
            if raw_d == "":
                continue
            try:
                val = int(raw_d.lstrip("+"))
                manual_delays[t["idx"]] = val
                print(f"    {G}✔ {lang_name} → {val:+d} ms{X}")
            except ValueError:
                print(f"    {Y}⚠ Invalid — skipped.{X}")
        ...
        if not _run_remux_with_titles(local_fp, tmp, sorted_audio, eng_subs, engine,
                                      manual_delays=manual_delays or None):
            local_fp.unlink(missing_ok=True)
            fail += 1
            print(f"\n{'─' * 68}")
            continue
...
def _banner():
    print(f"""{C}
╔══════════════════════════════════════════════════════════════╗
║    🎬📺 TAMIZHAN MOVIES — DOWNLOADER + AUTO-FIX  v9.4       ║
...
╚══════════════════════════════════════════════════════════════╝{X}
  ...
  Auto-Fix rules:
    ...
    🔊 Audio      → {G}Tamil first{X} (default+forced=✔), Tel→Mal→Kan→Hin→Eng→Kor→others, titles cleared
    💬 Subtitles  → {G}KEEP only{X} tam/tel/mal/kan/hin/eng/kor — all other languages {R}removed{X}
...
print(f"{G}✔ Tamizhan v9.4 loaded — all 6 menus ready.  ⚡ Smart-skip  🔁 Resume  🎵 A/V Sync Fix{X}")
main_menu()
