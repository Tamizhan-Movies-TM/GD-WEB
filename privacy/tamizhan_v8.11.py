# ╔══════════════════════════════════════════════════════════════════════╗
# ║   TAMIZHAN MOVIES — ULTRA DOWNLOADER + AUTO-FIX  v8.11               ║
# ║                                                                       ║
# ║  ALL MENUS USE FULL PARALLEL PIPELINES (up to 6 slots):              ║
...
# ║                                                                       ║
# ║  v8.11: Colab-native progress bars — each bar owns its own row,      ║
# ║         updates with \r (no ANSI cursor), no flood, no scroll.        ║
# ║         Connection pool 256 slots — no pool-full warnings.            ║
# ╚══════════════════════════════════════════════════════════════════════╝
...
# Shared HTTP session with large connection pool
# Pool size = THREADS (32) × max parallel downloads (6) = 192
# Suppress urllib3 "Connection pool is full" warnings at this size
import urllib3
...
_POOL_SIZE = 256   # comfortably above 32 chunks × 6 parallel files
...
# ══════════════════════════════════════════════════════════════════════
#  COLAB-COMPATIBLE PROGRESS BAR  (v8.11)
#
#  Colab's output cell is NOT a real TTY — ANSI cursor-up codes do
#  nothing.  Each \n is a permanent scroll.
#
#  Correct Colab approach:
#    • Each LiveBar prints ONE header line with \n (claims its row).
#    • Its background thread redraws ONLY that row using \r  so the
#      update happens in-place within that same cell output slot.
#    • A global print-lock (G_PRINT_LOCK) serialises all \r writes so
#      two bars never interleave on the same output line.
#    • Plain log messages (remux progress, errors, etc.) from other
#      threads are queued and printed AFTER the active bars finish,
#      not interleaved, so the bar display stays clean.
#    • Update rate capped at 2 Hz to avoid flooding Colab's output
#      buffer (which causes the massive scroll seen in v8.10).
# ══════════════════════════════════════════════════════════════════════

# Global print lock — all bar redraws and log flushes go through this
G_PRINT_LOCK = threading.Lock()

# Suppressed-log queue: messages emitted while bars are active are
# held here and printed in a clean block after all bars finish.
_LOG_QUEUE      = []
_LOG_QUEUE_LOCK = threading.Lock()
_BARS_ACTIVE    = [0]          # count of currently alive LiveBars
_BARS_CTR_LOCK  = threading.Lock()


def _colab_log(msg):
    """
    Thread-safe log print.
    If any bar is currently active the message is queued; otherwise
    it is printed immediately so it never cuts across a bar's \r line.
    """
    with _BARS_CTR_LOCK:
        active = _BARS_ACTIVE[0]
    if active > 0:
...
def _flush_log_queue():
    """Print all queued log messages.  Called when bar count hits 0."""
    with _LOG_QUEUE_LOCK:
        lines = _LOG_QUEUE[:]
        _LOG_QUEUE.clear()
    if lines:
...
# Compat alias — old code calls MultiBarManager.get().log(...)
class MultiBarManager:
    ...
    # Stubs kept for any remaining call sites
    def register(self, bar):   pass
    def unregister(self, bar, summary): pass


class LiveBar:
    """
    Colab-compatible single-line progress bar.

    Lifecycle:
      1. __init__  → prints a blank header line (\n) to claim the row,
                     starts a background refresh thread.
      2. update()  → accumulates bytes from any number of worker threads.
      3. _loop()   → every 0.5 s rewrites the claimed row with \r.
      4. finish()  → stops the loop, prints the final summary line.
    """

    BAR_WIDTH   = 32
    REFRESH_SEC = 0.5        # 2 Hz — fast enough, won't flood Colab
    ...
    def __init__(self, total_bytes, label="", phase=""):
        ...
        # Increment active-bar counter
        with _BARS_CTR_LOCK:
            _BARS_ACTIVE[0] += 1

        # Claim one output row; subsequent \r writes will update it
        with G_PRINT_LOCK:
            print(f"  {C}{self.label:<26}{X} [{' ' * self.BAR_WIDTH}] initialising...",
                  flush=True)

        threading.Thread(target=self._loop, daemon=True).start()

    # ── Data update (called from downloader threads) ─────────────────
    def update(self, n):
        with self._lock:
            self.done += n
            now = time.time()
    ...
    def _draw(self):
        with self._lock:
            ...
            bar    = f"{G}{'█' * filled}{'░' * (self.BAR_WIDTH - filled)}{X}"
            spd    = self._spd
            spd_s  = f"{spd / 1_048_576:5.1f} MB/s" if spd > 0 else " ?.? MB/s"
            eta    = (self.total - self.done) / spd if spd > 0 else 0
            ela    = int(time.time() - self._t0)
            ph     = f" {self.phase}" if self.phase else ""
        line = (
            f"\r  {C}{self.label:<26}{X}{ph} [{bar}]"
            f" {pct * 100:5.1f}%"
            f"  {_fmt_size(self.done)}/{_fmt_size(self.total)}"
            f"  {Y}{spd_s}{X}"
            f"  ETA {_fmt_eta(eta)}"
            f"  {ela}s   "
        )
    ...
    def finish(self, ok=True):
        self._alive = False
        time.sleep(self.REFRESH_SEC + 0.05)   # let loop exit cleanly
        ...
        with G_PRINT_LOCK:
            print(
                ...
                f"  {_fmt_size(self.total)}"
                f"  avg {avg:.1f} MB/s  {int(ela)}s          "
            )

        # Decrement active-bar counter; flush queued logs when last bar exits
        with _BARS_CTR_LOCK:
            _BARS_ACTIVE[0] = max(0, _BARS_ACTIVE[0] - 1)
            remaining = _BARS_ACTIVE[0]
        if remaining == 0:
            _flush_log_queue()

    # ── Retry-phase reset (called by downloader on fallback) ─────────
    def _restart_phase(self, new_phase):
        with self._lock:
...
def _drive_download_local(fid, fname, size):
    ...
    bar = LiveBar(size or 1, fname[:26], phase="⬇ DL→Local")
...
def _download_external(url, idx, total):
    ...
    phase = "↓ Download" if parallel else f"↓ Multi-Stream"
    ...
    try:
        if parallel and size > 0:
            ...
            if not parallel_ok:
                # Reset bar for retry with multi-stream
                bar._restart_phase(f"↓ Multi-Stream ({STREAM_CONNS})")
                tmp.unlink(missing_ok=True)
                try:
                    _multistream_dl(url, tmp, size, bar, STREAM_CONNS)
                except RuntimeError:
                    tmp.unlink(missing_ok=True)
                    bar._restart_phase("↓ Stream")
...
def _process_video_files_parallel(video_files, folder_name, folder_id, tok,
    ...
    def _plog(*args, **kw):
        _colab_log(' '.join(str(a) for a in args))
...
def run_fix_and_upload():
    ...
    def _plog(*args, **kw):
        """Thread-safe print — queued behind active download bars."""
        _colab_log(' '.join(str(a) for a in args))
        ...
        if local_zip.exists() and size > 0 and local_zip.stat().st_size == size:
            ...
            if parallel and size > 0:
                try:
                    ...
                    bar._restart_phase("↓ Multi-Stream")
...
def _banner():
    print(f"""{C}
╔══════════════════════════════════════════════════════════════╗
║   🎬📺 TAMIZHAN MOVIES — DOWNLOADER + AUTO-FIX  v8.11       ║
║   ⚡ All 5 menus parallel  📊 Colab-native bar display       ║
╚══════════════════════════════════════════════════════════════╝{X}
  Folder  : {Y}{FOLDER}{X}
...
print(f"{G}✔ Tamizhan v8.11 loaded — all 6 menus ready.  ⚡ Parallel pipelines  📊 Colab-native bars  🔌 Pool=256.{X}")
main_menu()
